...
static int expr()
{
    ...
    switch (token) {
        case '+':
            ...
            /* added a check for multiplication */
        case '*':
            next_token();
            left_reg = expr();
            right_reg = expr();
...
static void read()
{
    ...
    /* if the token is a ? , we need to go to the next token and geneate */
    if (token != '?') {
...
static void stmt()
{
    ...
    switch(token) {
        ...
        case 'e':
            assign();
            break;
            /* if its a ? we need to read the next token */
        case '?':
            read();
            break;
            /* if its a ! we need to print */
        case '!':
            print();
            break;
            /* if it is anything else, its an error */
        default:
            ERROR("Expected statement (assign, print, or read)\n");
            exit(EXIT_FAILURE);
    }
}
